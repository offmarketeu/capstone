...
write.csv(tkdfm,paste(dir3,"tokens1.csv", sep="/"))

#2-grams
tkdfm2 <- dfm(fullcorpues, tolower=TRUE, remove_numbers = TRUE, remove_punct=TRUE, ngrams = 2, verbose = FALSE)
top_ng2<- topfeatures(tkdfm2)
freq_2 <- data.frame(word=names(top_ng2), count=top_ng2)
write.csv(tkdfm2,paste(dir3,"tokens2.csv", sep="/"))


#3-grams
tkdfm3 <- dfm(fullcorpues, tolower=TRUE, remove_numbers = TRUE, remove_punct=TRUE, ngrams = 3, verbose = FALSE)
top_ng3<- topfeatures(tkdfm3)
freq_3 <- data.frame(word=names(top_ng3), count=top_ng3)
write.csv(tkdfm3,paste(dir3,"tokens3.csv", sep="/"))
